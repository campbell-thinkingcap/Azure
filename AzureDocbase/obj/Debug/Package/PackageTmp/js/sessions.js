...
*/
function count(filterQuery, continuationToken, uniqueClient) {
    ...
    var result = 0;
    var clients = [];
    var temp = uniqueClient;
    tryQuery(continuationToken, temp);
    ...
    function onReadDocuments(err, docFeed, responseOptions) {
        ...
        for (var xx = 0; xx < docFeed.length; xx++) {
            if (docFeed[xx].SessionID !== temp && docFeed[xx].SessionID != "")
                clients.push('{"id":"' + docFeed[xx].SessionID + '","time":"'+docFeed[xx].Timestamp + '","student":"'+docFeed[xx].StudentName +'"}');
            temp = docFeed[xx].SessionID;
        }
    ...
    function setBody(continuationToken) {
        var body = { count: result, continuationToken: continuationToken, sessions: clients, uniqueClient: temp };
