...
*/
function Sessions(filterQuery, continuationToken,uniqueClient) {
    ...
    function onReadDocuments(err, docFeed, responseOptions) {
        ...
        for (var xx = 0; xx < docFeed.length; xx++) {
            if(docFeed[xx].client !== temp)
                clients.push(docFeed[xx].client);
            temp = docFeed[xx].client;
        }
    ...
    function setBody(continuationToken) {
        var body = { count: result, continuationToken: continuationToken, clients : clients, uniqueClient : temp };
